...
export function ResumeScorer({ resume, onScoreUpdate }: ResumeScorerProps) {
  ...
  const generateScoreReport = (resume: ResumeData): ScoreReport => {
    ...
    const maxScore = categories.reduce((sum, cat) => sum + (cat.maxScore * cat.weight), 0);
  ...
  // 分析个人信息
  const analyzePersonalInfo = (personalInfo: ResumeData['personalInfo']): ScoreCategory => {
  ...
  // 生成优势
  const generateStrengths = (resume: ResumeData, categories: ScoreCategory[]): string[] => {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">全面分析简历质量，提供改进建议</p>
          </div>
        </div>
        
        <Button onClick={analyzeResume} disabled={isAnalyzing || !resumeData} className="gap-2">
          {isAnalyzing ? (
          ...
          <CardContent className="p-6">
            <div className="text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">请稍候，正在进行全面质量评估</p>
              </div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="text-center">
                  <div className={`text-4xl font-bold ${getGradeColor(scoreReport.grade)}`}>
                    {scoreReport.grade}
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">{scoreReport.readabilityScore}/100</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">关键词密度</span>
                  <span className="font-medium">{scoreReport.keywordDensity}%</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">预估查看时间</span>
                  <span className="font-medium">{scoreReport.estimatedViewTime}分钟</span>
                </div>
              </div>
            </CardContent>
          </Card>
                    ...
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant={category.score / category.maxScore >= 0.8 ? "default" : "secondary"}>
              ...
              <div className="grid grid-cols-1 gap-4">
                {scoreReport.categories.map((category) => (
                  category.suggestions.length > 0 && (
                    <Card key={category.name}>
                      <CardHeader>
                        <CardTitle className="text-lg">{category.name} - 改进建议</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <ul className="space-y-2">
